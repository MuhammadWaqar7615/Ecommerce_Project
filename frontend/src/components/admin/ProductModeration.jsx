...
import {
  ...
  Package, Image, ChevronLeft, ChevronRight,
...
const ProductModeration = () => {
  ...
  const [viewMode, setViewMode] = useState(() => {
    return localStorage.getItem('adminProductViewMode') || 'table';
  });
  ...
  const saveScrollPosition = useCallback(() => {
    sessionStorage.setItem('adminProductListScrollY', window.scrollY);
    sessionStorage.setItem('adminProductListState', JSON.stringify({
      viewMode,
      currentPage,
      searchTerm,
      vendorFilter,
  ...
  }, [viewMode, currentPage, searchTerm, vendorFilter, statusFilter, categoryFilter, stockFilter, sortBy]);

  // Restore scroll position when coming back
  useEffect(() => {
    const savedState = sessionStorage.getItem('adminProductListState');
    const savedScrollY = sessionStorage.getItem('adminProductListScrollY');
    ...
    return () => {
      sessionStorage.removeItem('adminProductListState');
      sessionStorage.removeItem('adminProductListScrollY');
    };
  }, []);

  // Save view mode to localStorage
  useEffect(() => {
    localStorage.setItem('adminProductViewMode', viewMode);
  }, [viewMode]);
  ...
  useEffect(() => {
    ...
    if (vendorFilter) {
      filtered = filtered.filter(product =>
        product.shopId?._id === vendorFilter ||
        product.shopId?.shopName === vendorFilter
      );
    }
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'vendor_asc':
          return (a.shopId?.shopName || '').localeCompare(b.shopId?.shopName || '');
        default:
          return 0;
      }
    });
    ...
    setCurrentPage(1);
  }, [products, searchTerm, vendorFilter, statusFilter, categoryFilter, stockFilter, sortBy]);
  ...
  const fetchVendorsAndCategories = async () => {
    try {
      ...
      // Fetch vendors
      const vendorsResponse = await fetch('http://localhost:5000/api/admin/vendors', {
  ...
  const hasActiveFilters = searchTerm || vendorFilter || statusFilter || categoryFilter || stockFilter || sortBy !== 'newest';
  ...
  const getStockStatus = (stock) => {
    if (stock === 0) return { label: 'Out of Stock', color: 'red', bg: 'bg-red-50', text: 'text-red-700' };
    if (stock <= 10) return { label: 'Low Stock', color: 'orange', bg: 'bg-orange-50', text: 'text-orange-700' };
    return { label: '', color: 'green', bg: 'bg-green-50', text: 'text-green-700' };
  };
