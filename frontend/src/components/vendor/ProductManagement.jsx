...
const ProductManagement = () => {
    ...
    return (
        ...
        >
            ...
            {/* Header */}
            <motion.div variants={cardVariants} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 md:p-6">
                <div>
                ...
                <div className="flex items-center w-full justify-between md:w-fit md:justify-end gap-3">
                    <div className="flex bg-gray-100 rounded-lg p-1">
                        <button onClick={() => setViewMode('table')} className={`p-2 rounded-md transition ${viewMode === 'table' ? 'bg-white shadow-sm text-primary' : 'text-gray-500'}`}>
                            <Table2 size={18} />
                        </button>
                        <button onClick={() => setViewMode('grid')} className={`p-2 rounded-md transition ${viewMode === 'grid' ? 'bg-white shadow-sm text-primary' : 'text-gray-500'}`}>
                            <Grid3x3 size={18} />
                        </button>
                    </div>
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => navigate('/vendor/products/add')} className="text-sm md:text-base btn-primary flex items-center gap-2 px-5 py-2.5">
                        <Plus size={18} />
                        Add Product
                    </motion.button>
                </div>
